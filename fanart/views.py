...
from fanart.models import Contest, Favorite, TradingOffer, TradingClaim, ColoringBase
...
class UserBoxView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['icons_mine'] = TradingClaim.objects.filter(offer__type='icon', offer__is_active=True, offer__is_visible=True, date_fulfilled__isnull=True, filename='', offer__date_posted__gt=icons_publish_start_date, user=self.request.user)
        context['adoptables'] = TradingOffer.objects.filter(type='adoptable', is_active=True, is_visible=True, date_posted__gt=adoptables_publish_start_date)
        context['adoptables_unclaimed'] = context['adoptables'].filter(adopted_by__isnull=True)
        context['adoptables_mine'] = TradingClaim.objects.filter(offer__type='adoptable', offer__is_active=True, offer__is_visible=True, date_fulfilled__isnull=False, offer__date_posted__gt=adoptables_mine_start_date, user=self.request.user)
        context['coloring_bases'] = ColoringBase.objects.filter(is_active=True, is_visible=True)

        return context


class HomeView(UserBoxView):
